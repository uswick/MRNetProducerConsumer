...
const char *IntegerStreamAggregator_format_string = "%d";
...
void IntegerStreamAggregator(std::vector< PacketPtr > &packets_in,
...
//    sleep(5000);
    ...
    if (state->prod->getWaitCondition()) {
        ...
        //consume output
        std::vector<int> *outQ = state->th->getOutputQueue();
        int size = outQ->size();
        int *sums = new int [size];
        int i = 0;
        std::vector<int>::iterator del;
        //drain from ouput-queue
        for (std::vector<int>::iterator it = outQ->begin(); it != outQ->end();) {
            sums[i] = *it;
            del = it;
            it = outQ->erase(del);
            i++;
