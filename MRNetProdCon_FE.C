...
int main(int argc, char **argv)
{
    ...
    while( n++ < nets ) {
        ...
        fprintf(stdout, "PID: %d top : %s , BE : %s dummy : %s \n", getpid(),
        topology_file, backend_exe, dummy_argv);
        ...
        int filter_id = net->load_FilterFunc( so_file, "IntegerStreamAggregator" );
        if( filter_id == -1 ){
...
//                return -1;
//            }
            if( p->unpack( "%ad", &recv_Ar, &length ) == -1 ){
                fprintf( stderr, "stream::unpack() failure\n" );
                return -1;
            }
